...
class UserOperations extends ResourceCollection
{
    private $user;
    ...
    public function toArray($request)
    {
        ...
        return[
            ...
            // "super_likes_left"=> Like::where("liker_user_id",  $this->user)->where("super_like", "=", 1)->where("created_at", ">", Carbon::now()->subDays(1))->count(),
            // //"boost_left"=> BoostController::leftBoost($this->user),
            // "likes_left"=>Like::where("liker_user_id",  $this->user)->where("super_like", "=", 0)->where("created_at", ">", Carbon::now()->subDays(1))->count(),
            // // "free_likes_per_day"=>$setting->free_likes_per_day,
            // // "free_boosts_per_month"=>$setting->free_boosts_per_month,
            // // "free_super_likes_per_month"=>$setting->free_super_likes_per_month//::freeSuperLikeLeft($this->user)
        ];
    }
